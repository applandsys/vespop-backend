...
} = require("@/modules/product/model/categoryModel");
...
const addProductCategory = async (req, res) => {
    try {
        ...
        const image = req.uploadedFiles?.image || null;
        const icon = req.uploadedFiles?.icon || null;
        ...
        /*
        {
              "image": "https://cdn.yourdomain.com/categories/images/uuid.jpg",
              "icon": "https://cdn.yourdomain.com/categories/icons/uuid.png"
            }
         */
    ...
    try {
      ...
      //  const images = req.files?.["image"] || [];
        ...
        if (productId) {
            ...
            // Parse images user wants to keep
            const keepImageIds = safeParseJsonArray(existingImages)
                .map((img) => img?.id)
                .filter(Boolean);

// Get all current images from DB
...
// 🔥 Delete removed images from R2
            for (const img of removedImages) {
                try {
                    await deleteFromR2(img.name); // name = CDN URL
                } catch (err) {
                    console.warn('R2 delete failed (skipped):', img.name);
                }
            }
...
const addProductBrand = async (req, res) => {
    try {
        ...
        // 🖼 R2 OR fallback file
        const logo = req.brandLogo || null;

        // 🧠 Prevent duplicate slug issue
        const slug = generateSlug(name);
        ...
        const newBrandCreated = await prisma.productBrand.create({
            data: {
                ...
                logo, // ✅ store CDN URL (NOT filename)
            },
        });
