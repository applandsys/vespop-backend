...
const {getOrderListWithCount} = require("@/modules/order/model/orderModel");
   ...
   const {orderId} = req.body;
    try{
        ...
        const payOrder = async (req, res) => {
            const order = await markOrderPaid(req.params.id, req.user?.id);
...
const orderReturnList = async (req, res) => {
    const type = req.params.type || null;
    try{
        const orders = await getOrderList(type);
        res.status(201).json({
